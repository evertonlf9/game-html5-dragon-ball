var game = (function() {
	...
	var game = {
		...
		init: function(  ) {
			...
			this.jogo = $( "#start" );
			$( this.jogo ).click( function(){ game.onload.messageBox(); } );
	...
	/*-----------------------------------------------------------------------*/
		...
		armazenarImg: function( index, imgSrc ){
			...
			var tile = [ index, new Image(), false ]; // format as explained: [id#, Image, loaded?]
			...
			this.imgArr[ index ] = tile; // Armazena este tile
		},//store
	...
	game.criaMapa = {
		...
		drawShenlong: function(){
			...
			if( game.shenlongDraw ){
				this.drawCanvas( game.shenlong.sprite, ( game.shenlong.CurrPosX * 150 ) , game.shenlong.CurrPosY, 
								 game.shenlong.width, game.shenlong.height, game.shenlong.x, game.shenlong.y );

				game.animate.beginAnimate(  game.shenlong );
				// this.drawCanvas( game.onload.recuperarImg( 9 ), 0, 0, 150, 107, 358, 100 );
			}//if
		...
		CriandoMapa: function(){
			...
			for( lin = 0; lin < game.jsonData[game.strMapa].length ; lin++ )
		    {
			   ...
			   {	
				  ...
				  this.drawTile( col, lin, tile.ground );
	...
	/*-----------------------------------------------------------------------*/
		...
		drawEnemy: function (  ){
			...
			for( var i = 0; i < game.arrEnemy.length; i++ ){		   
				...
				this.drawCanvas( game.arrEnemy[ i ].sprite, SpritePosX, SpritePosY, game.tamTile, game.tamTile, 
			    ...
			    {	
				  tile = ( game.jsonData[game.strMapa][ lin ] && game.jsonData[game.strMapa][ lin ][ col ] ) ?  game.jsonData[game.strMapa][lin ][ col ]: {index: 0}; 
				    ...
				    if( game.jsonData[game.strMapa][ lin ][ col ].index != 0 )
						this.drawTileIndex( col , lin, tile.index );
		...
		drawPlayer: function (  ){
		   ...
		   SpritePosY = game.player.CurrPosY * game.tamTile;
		    ...
		    if( game.player.status ){
		   	...
		   	this.drawCanvas( game.player.sprite, SpritePosX, SpritePosY, game.tamTile, game.tamTile, 
	...
	/*-----------------------------------------------------------------------*/
		 ...
		 * Desenha o tile do mapa na tela
	 	 */
		drawTile: function ( x, y, tileSheet  ){   	 
			...
			this.drawCanvasFundo( game.tileShet, CoordX * game.tamTile, CoordY * game.tamTile, game.tamTile, game.tamTile,  x * game.tamTile, y * game.tamTile );
	...
	/*-----------------------------------------------------------------------*/
		 ...
		 * Desenha o tile do mapa na tela
	 	 */
		drawTileIndex: function ( x, y, tileSheet ){   	 
			...
			game.tileShet = game.onload.recuperarImg( 0 );
			this.drawCanvas( game.tileShet, CoordX * game.tamTile, CoordY * game.tamTile, game.tamTile, game.tamTile,  x * game.tamTile, y * game.tamTile );
	...
	/*-----------------------------------------------------------------------*/
		...
		drawTiro: function( tiroArray, type ){
		   ...
		   for( var i = 0; i < tiroArray.length; i++ ){		   
			  ...
			  this.drawCanvas( tiroArray[ i ].spritSheet, Sprite, 0, game.tamTile, game.tamTile, tiroArray[ i ].x , tiroArray[ i ].y  );
			  // game.ctx.strokeRect (tiroArray[ i ].x, tiroArray[ i ].y, 32, 32);
			  if( type == "player" ){
	...
	/*-----------------------------------------------------------------------*/
		...
		drawItem: function (  ){
			...
			for (var i = 0; i < game.itemJogo.length; i++) {		
				...
				this.drawCanvas( game.itemJogo[i].sprite, ( game.itemJogo[i].tipoItem * game.tamTile), 0, game.tamTile, game.tamTile, 
	...
	/*-----------------------------------------------------------------------*/
		 ...
		 **/
		drawCanvas: function ( objSprite, initX, initY, widthX, heightY, x, y ){			
			...
			game.ctx.drawImage( objSprite, initX, initY, widthX, heightY, x, y, widthX, heightY );
	...
	/*-----------------------------------------------------------------------*/
		 ...
		 * Desenha as imagens no canvas 
		 **/
		drawCanvasFundo: function ( objSprite, initX, initY, widthX, heightY, x, y ){	
			//----------------------altera aqui pra mexer na animaçao da imagem-------------------------
			// imagem a ser impressa, posX a ser exibida da img, posY a ser exibida da img, Widht exibido dela, height exibido dela, posX na tela, 
			// posY na tela, width da img ja cortada, height da img ja cortada		
			game.ctxFundo.drawImage( objSprite, initX, initY, widthX, heightY, x, y, widthX, heightY );
	...
	/*-----------------------------------------------------------------------*/
		...
		drawtext: function( x, y ){
			...
			game.ctx.fillText( game.vDanoFinal + " Hit", x, y ); 
	...
	/*-----------------------------------------------------------------------*/
		...
		colisaoEnemy: function( objTiro ){			
			...
			{			
				...
				{
					this.destroyObj( objTiro, game.arrTiro );
					// game.criaMapa.drawtext( objTiro.x, objTiro.y );
